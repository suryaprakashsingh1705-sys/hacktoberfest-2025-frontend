 ...
 * - filterFn: optional function(product) => boolean to pre-filter products
 * - saleOnly: boolean - if true will render RecentlyViewed with saleOnly
 */
export default function ProductList({
  ...
  // Combined filtering: global filters + optional external filterFn
  ...
  }, [allProducts, filters, filterFn, maxProductPrice]);
  ...
  return (
    <>
      <SEO
        title={seo.title || `${title} | CoreX Nutrition`}
        description={
          seo.description ||
          'Browse our products and find what supports your fitness goals.'
        }
      ...
      <main className={`min-h-screen bg-[#F7FAFF] ${isFilterOpen ? 'relative' : ''}`}>
        ...
        {/* Banner: show image banner if provided, otherwise default text banner */}
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center">
                ...
                <p className="text-xl mb-8 text-blue-100 max-w-2xl mx-auto">
                  {seo.description || 'Discover our premium product range.'}
                </p>
              </div>
            </div>
          </section>
        ...
        {!loading && !error && <RecentlyViewed saleOnly={saleOnly} />}
      </main>
    </>
  );
}
